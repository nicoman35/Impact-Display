/*
	...
	Description:
		...
		it will constantly choeck the world coordinates of the mouse position. The position is then 
...
waitUntil {time > 0};
sleep random 5;
if (player getVariable "NIC_IMP_DSP_MonitorCmdMenuMutex") exitWith {};
...
while {NIC_IMP_DSP_MONITOR_CMD} do {																	// while monitoring of player command menu allowed
	...
	{
		_vehicle = vehicle _x;
		// if (_vehicle != _x && (_x == gunner _vehicle || _x == commander _vehicle)) then {
		if (_vehicle != _x && _x == gunner _vehicle) then {
			{
				if (_vehicle isKindOf _x) exitWith {
					// _vehicle setVariable ["NIC_IMP_DSP_cursorData", _positionCrosshair];					// if a unit is in a vehicle listed in NIC_IMP_DSP_MONITORED_VEHICLES, memory coordinates on that vehicle
					...
					// diag_log formatText ["%1%2%3%4%5%6%7%8%9", time, "s  _vehicle: ", _vehicle, " is of kind: ", _x, " _coordinates: ", _coordinates];
				};
