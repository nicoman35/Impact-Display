...
private ["_vehicle", "_positionCrosshair", "_cursorObject", "_initialPositionObject"];
while {NIC_IMP_DSP_MONITOR_CMD} do {																												// while monitoring of player command menu allowed
	...
	while {commandingMenu != ""} do {																												// while command menu is open
		if (count groupSelectedUnits player > 0) then {
			...
			_positionCrosshair = screenToWorld [0.5, 0.5];																							// monitor position coordinates of targeting cursor																				// monitor position coordinates of targeting cursor
			if (visibleMap) then {
