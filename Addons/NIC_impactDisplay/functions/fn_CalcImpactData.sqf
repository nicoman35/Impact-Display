...
while {_i < 5} do {
	...
	_range = _Vzero * cos _elevAnkle * (_Vzero * sin _elevAnkle + sqrt((_Vzero * sin _elevAnkle)^2 + 2 * 9.81 * _heightDiff)) / 9.81;			// calculate range of round. Formula: y = h + x * tan(α) - g * x² / 2 * V0² * cos²(α). Source: https://www.omnicalculator.com/physics/trajectory-projectile-motion#trajectory-formula
...
// to do: 	- replace wrong formula sources with right one
// 			- calculate ETA with time of flight, as getArtilleryETA might sometimes give wrong values

NIC_IMP_DSP_fnc_CalcImpactData_dbg = {
	params [["_projectile", [objNull]]];
	if (isNull _projectile) exitWith {[]};

	// Calculate time of flight. Formula: t = 2 * V0 * sin(α) / g. Source: https://www.omnicalculator.com/physics/trajectory-projectile-motion#trajectory-formula
};
